...
| 阶段 | 主题 | 状态 | 估时 | 验收标准 | 详情 |
|---|---|---|---|---|---|
| **A** | Agent 开发基建（先于一切 UI 改动） | 🟡 in-progress | 1.5 天 | `npm run check:e2e` 在 CI 全绿 | [phase-A-dev-infra.md](./phase-A-dev-infra.md) |
| **0** | shadcn 接入 + token 对齐 + 去 glass | ⏳ pending | 1 天 | `ui/Button` 替换为 shadcn 版本，token 全部对齐，glass 主题完全移除 | [phase-0-shadcn.md](./phase-0-shadcn.md)（待写） |
